...
class LeaveApplicationEmpTable extends Component {
  ...
  loadLeaveApplicationEmpData = () => {
    axios
      .get(
        (process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/leave-application-emp/" +
        this.props.data["_id"], {
        headers: {
  ...
  onLeaveApplicationEmpDelete = (e1, e2) => {
    ...
    if (window.confirm("Are you sure to delete this record? ") == true) {
      axios
        .delete(
          (process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/leave-application-emp/" + e1 + "/" + e2, {
          headers: {
            authorization: localStorage.getItem("token") || ""
          }
        }
