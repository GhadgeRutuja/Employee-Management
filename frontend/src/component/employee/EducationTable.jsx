...
class EducationTable extends Component {
  ...
  loadEducationData = () => {
    axios
      .get((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/education/" + this.props.data["_id"], {
        headers: {
  ...
  onEducationDelete = (e1, e2) => {
    if (window.confirm("Are you sure to delete this record? ") == true) {
      axios
        .delete((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/education/" + e1 + "/" + e2, {
          headers: {
            authorization: localStorage.getItem("token") || ""
          }
        })
