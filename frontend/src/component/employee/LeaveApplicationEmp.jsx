...
class LeaveApplicationEmp extends Component {
  ...
  handleLeaveApplicationEmpSubmit = event => {
    ...
    axios
      .post((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/leave-application-emp/" + this.props.data["_id"], body, {
        headers: {
  ...
  handleLeaveApplicationEmpEditUpdate = (info, newInfo) => {
    ...
    axios
      .put(
        (process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/leave-application-emp/" + info["_id"],
        body, {
        headers: {
          authorization: localStorage.getItem("token") || ""
        }
