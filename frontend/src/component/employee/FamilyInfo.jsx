...
class FamilyInfo extends Component {
  ...
  handleFamilyInfoSubmit = event => {
    ...
    axios
      .post((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/family-info/" + this.props.data["_id"], body, {
        headers: {
  ...
  handleFamilyInfoEditUpdate = (info, newInfo) => {
    ...
    axios
      .put(
        (process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/family-info/" + info["_id"],
        body, {
        headers: {
          authorization: localStorage.getItem("token") || ""
        }
