...
class AdminProjectBid extends Component {
  ...
  handleProjectBidSubmit = event => {
    ...
    //  let body= "";
    axios
      .post((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/admin/project-bid", body, {
        headers: {
  ...
  handleProjectBidEditUpdate = (info,editInfo) => {
    ...
    axios
      .put((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/admin/project-bid/" + info["_id"], body, {
        headers: {
          authorization: localStorage.getItem("token") || ""
        }
      })
