...
class LeaveApplicationHR extends Component {
  ...
  handleLeaveApplicationHRSubmit = event => {
    ...
    axios
      .post((process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/leave-application-hr/" + this.props.data["_id"], body, {
        headers: {
  ...
  handleLeaveApplicationHREditUpdate = (info, newInfo) => {
    ...
    axios
      .put(
        (process.env.REACT_APP_API_URL || "http://localhost:4000") + "/api/leave-application-hr/" + info["_id"],
        body, {
        headers: {
          authorization: localStorage.getItem("token") || ""
        }
